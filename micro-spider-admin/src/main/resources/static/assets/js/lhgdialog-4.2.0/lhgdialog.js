...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		DOM.wrap.addClass( "anim-target anim-open" ); //装载CSS3动画
	...
	button: function()
	{
		...
		for( ; i < ags.length; i++ )
		{
			...
			if( item.className )
			{
				$(button).removeClass().addClass(item.className);
			}
 ...
 * @param	{Function}	取消按钮回调函数
 */
lhgdialog.confirmDanger = function( content, yes, no, parent )
{
	return lhgdialog({
		title: '确认',
		id: 'confirm',
