package com.yaofei.framework.excel;

import com.yaofei.framework.util.DateUtils;
import com.yaofei.framework.util.ReflectUtils;
import com.yaofei.framework.util.StringUtils;
	...
	public <T> List<T> readExcel(Class<T> clazz, String[] fieldNames,
		...
		for (int i = start; i <= sheet.getLastRowNum(); i++) {
			...
			for (int j = 0; j < fieldNames.length; j++) {
				...
				if(cell.getCellType()==XSSFCell.CELL_TYPE_NUMERIC&&content.contains("E10"))
				{
					DecimalFormat df = new DecimalFormat("#"); //可避免科学计数
					content=df.format(cell.getNumericCellValue());
				}
				// 如果属性是日期类型则将内容转换成日期对象
				if (isDateType(clazz, fieldName)) {
					// 如果属性是日期类型则将内容转换成日期对象
					ReflectUtils.invokeSetter(target, fieldName,
		...
		switch (cell.getCellType()) {
			case XSSFCell.CELL_TYPE_NUMERIC : // 数字
				if (HSSFDateUtil.isCellDateFormatted(cell)) {
					...
					String value= DateFormatUtils.format(date, "yyyy-MM-dd HH:mm:ss:SSS");
					buffer.append(value);
				} else {
					double d = cell.getNumericCellValue();
					if (d - (int) d < Double.MIN_VALUE) {
						// 是否为int型
						buffer.append(Integer.toString((int) d));
					} else {
